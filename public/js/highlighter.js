...
(function () {
  ...
  const rTokens = [
    [
      ...
      [
        "keyword",
        /\b(?:break|case|catch|continue|debugger|default|delete|do|else|finally|for|function|if|in|instanceof|new|return|switch|this|throw|try|typeof|var|void|while|with)\b/,
      ],
      ["number", /(?=[\-+])(?:[\-+]?\d+(?:\.\d+)?)|(?:\b\d+(?:\.\d+)?)/],
      ["object", /(?:^|\b)\w[\w\d]+(?=\.)/],
      ["callable", /(?:^|\b)\w[\w\d]+(?=\s*?\()/],
    ...
    (tokens) =>
      tokens.map(([key, regex], index) => [
        key,
        unwrap(regex).replace(
          /\d/g,
          (match) => `${parseInt(match) + index + 1}`
        ),
      ]),

    (tokens) => tokens.map(([name, regex]) => `(?<${name}>${regex})`).join("|"),
  ...
  function init() {
    Array.from(
      document.querySelectorAll(
        'pre code[class="language-js"], pre code[class="language-javascript"]'
      )
    ).forEach(highlight);
  }
