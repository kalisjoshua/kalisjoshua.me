(function () {
  const addToHistory = debounce((slideID) =>
  ...
  function makeSlides(rawContent, header) {
    ...
    return rawContent
      ...
      .map((html, index, { length: slideCount }) => {
        ...
        footer.append(...header.cloneNode(true).children, counter);
  ...
  function scrollObserverSetup() {
    const callback = (slides) => {
      ...
      slides
        ...
        .forEach((slide) => {
          ...
          Array.from(slide.target.children).slice(-1)[0].style.opacity =
            ratio < 0.8 ? ratio / 4 : ratio;
        });

      if (active && active.intersectionRatio > 0.6) {
    ...
    Array.from(options.root.children).forEach((slide) =>
      scrollObserver.observe(slide)
    );
  }

  rSlidesURL.test(window.location) &&
